...
- The verdict MUST OPEN with the gap statement in plain numbers: "You earned $X this window; your net worth moved $Y — here's the $Z difference:" then give THE SIMPLE STORY in one or two sentences — the 1-2 real causes and what to do.
- SIMPLICITY RULE — compute exactly, then tell it simply. Most gaps are one or two REAL causes plus self-correcting noise. Check HAND-TYPED entries FIRST (manual card lines, manually-added ad spend, typed balances) — they are suspect #1 and usually ARE the story (e.g. a gap of $379 that is simply $360 of manually-added Google spend + an $81 hand-typed Google card line). Order causes by what the owner should act on, not by dollar size.
- FOLD THE NOISE: all self-correcting timing (payout pipeline, billing lag) goes into ONE combined cause line ("normal payout/billing timing — self-corrects, nets to $X"). If two large items roughly cancel (e.g. a baseline artifact vs pipeline timing), present them as ONE line saying they cancel — never as two separate headline findings.
- Your causes must still SUM to the gap (gap_cents). Report unexplained_remaining_cents as gap minus your causes.
