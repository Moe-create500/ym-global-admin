...
export async function getAdSets(
  ...
  if (!res.ok) throw new Error(`FB getAdSets failed: ${await res.text()}`);
  const data = await res.json();
  return data.data || [];
}
...
export async function createCampaign(
  ...
  try {
    return await fbPost(`${adAccountId}/campaigns`, accessToken, payload);
  } catch (err: any) {
    ...
    if (isObjectiveError && objective === 'OUTCOME_SALES') {
      ...
      try {
        return await fbPost(`${adAccountId}/campaigns`, accessToken, {
          ...payload,
...
export async function createAdSet(
  ...
  return fbPost(`${adAccountId}/adsets`, accessToken, body);
}
