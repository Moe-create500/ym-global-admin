 ...
 * If no imageUrl provided, uses text-only generation.
 */
export async function createVideo(
  ...
  // Only include input_images if we have a real image URL
  ...
  const endpoint = (imageUrl && imageUrl.startsWith('http')) ? '/v1/image2video/dop' : '/v1/text2video/dop';
  console.log(`[HIGGS] ${endpoint}: prompt=${prompt.substring(0, 80)}... ${imageUrl ? 'with image' : 'text-only'}`);

  const data = await higgsFetch(endpoint, 'POST', { params });
