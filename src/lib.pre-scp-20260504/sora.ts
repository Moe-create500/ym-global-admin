...
async function soraFetch(endpoint: string, method: 'GET' | 'POST' = 'GET', body?: any) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`Sora API error ${res.status}: ${text}`) as any;
    err.status = res.status;
    err.isQuota = res.status === 429 || text.includes('quota') || text.includes('billing');
    throw err;
  }
...
export async function createVideo(
  ...
  // Prepend silent-audio instruction to prompt — Sora generates glitchy ambient sounds otherwise
  const silentPrompt = `SILENT VIDEO — absolutely NO audio, NO sound effects, NO ambient noise, NO music, NO room tone, NO hum. Complete silence. ${prompt}`;

  if (options.imageBuffer) {
    // Use multipart form-data to send the pre-resized image as input_reference
    const formData = new FormData();
    formData.append('model', model);
    formData.append('prompt', silentPrompt);
    ...
    const body: any = { model, prompt: silentPrompt, size, seconds };
    if (options.imageUrl) {
