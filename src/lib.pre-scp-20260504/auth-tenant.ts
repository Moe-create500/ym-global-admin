...
/**
 * Require admin or data_corrector role. Use for routes that perform
 * destructive or bulk-write operations across stores.
 * Returns the session if authorized, or 401/403 NextResponse if not.
 */
export function requireAdmin(
  ...
  if (session.role !== 'admin' && session.role !== 'data_corrector') {
    console.log(`[AUTH] BLOCKED admin-only: employee ${session.employeeId} (${session.role})`);
