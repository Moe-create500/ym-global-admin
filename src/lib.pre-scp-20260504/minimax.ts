...
export async function createVideo(
  ...
  // Validate duration: MiniMax supports 5 or 6 seconds only
  const rawDuration = options.duration || 6;
  const duration = rawDuration <= 5 ? 5 : 6;
  // Validate resolution: only 720P or 1080P
  const resolution = options.resolution === '720P' || options.resolution === '720p' ? '720P' : '1080P';

  const payload: any = {
    model,
    prompt: prompt.substring(0, 2000), // MiniMax prompt limit
    duration,
    resolution,
  };
  ...
  console.log(`[MINIMAX] Creating video: model=${model}, duration=${duration}s, resolution=${resolution}, prompt=${prompt.substring(0, 80)}...`);
