  ...
  try {
    ...
    for (const [day, rev] of Object.entries(dailyRevMap)) {
      ...
      if (existing) {
        ...
        const totalCosts = productCost + fulfillmentCharges + adSpend + shopifyFees + otherCosts + chargebacks + appCosts;
        ...
        const totalCosts = productCost + fulfillmentCharges;
...
export async function syncFacebookAds(maxAgeMinutes?: number): Promise<{ synced: number; invoicesImported: number; errors: string[] }> {
  ...
  for (const profile of profiles) {
    // First sync: use store's sync_start_date (or 90 days). Subsequent: last 30 days.
    ...
    let from = profile.last_sync_at
      ...
      : (store?.sync_start_date || pacificDate(Date.now() - 90 * 86400000));
    ...
    try {
      ...
      for (const insight of insights) {
        ...
        if (adId) {
          ...
          db.prepare(`DELETE FROM ad_spend WHERE store_id = ? AND date = ? AND platform = 'facebook' AND campaign_id = ? AND ad_id IS NOT NULL AND ad_id IS NULL`)
